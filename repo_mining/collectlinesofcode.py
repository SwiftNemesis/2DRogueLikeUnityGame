...
def get_repo_data(username, repo_name, token):
    ...
    for pr in prs:
        ...
        if users[user_login] not in repo_data:
            repo_data[users[user_login]] = pr_detail['additions'] + pr_detail['deletions']
        else:
            repo_data[users[user_login]] += pr_detail['additions'] + pr_detail['deletions']
...
def create_graph(repo_data):
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(x, changed_files, width, label='Changed Lines of Code')

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Count')
    ax.set_title('Changed Lines of Code by User')
    ax.set_xticks(x)
